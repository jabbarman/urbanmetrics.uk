...
export function MetricCards({ catalog, status, activeLayer }: MetricCardsProps) {
  ...
  const primaryGeography = activeLayer?.geographyLabel ?? "Mixed layer geographies";
  ...
  return (
    <div className="grid gap-3 md:grid-cols-4">
      <article className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_12px_40px_rgba(15,23,42,0.08)]">
        ...
        <p className="mt-2 text-sm text-slate-600">Generated comparison layers for the current observatory workspace.</p>
      </article>
      <article className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_12px_40px_rgba(15,23,42,0.08)]">
      ...
      <article className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_12px_40px_rgba(15,23,42,0.08)]">
        ...
        <p className="mt-3 text-2xl font-semibold text-slate-950">{primaryGeography}</p>
        <p className="mt-2 text-sm text-slate-600">Shown for the active primary layer so the map context stays explicit.</p>
      </article>
      <article className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_12px_40px_rgba(15,23,42,0.08)]">
