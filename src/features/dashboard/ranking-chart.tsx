    ...
    <section className="rounded-[1.75rem] border border-slate-200/80 bg-white p-6 shadow-[0_12px_40px_rgba(15,23,42,0.08)]">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-lg font-semibold text-slate-950">Highest-value areas for {layer.layer.shortLabel}</h2>
        </div>
      ...
      <div className="mt-6 h-72 w-full">
        <ResponsiveContainer>
          <BarChart data={data} layout="vertical" margin={{ top: 0, right: 12, bottom: 0, left: 16 }}>
            ...
            <Tooltip cursor={{ fill: "rgba(13, 79, 118, 0.08)" }} formatter={(value: number) => `${value}${layer.layer.unit === "%" ? "%" : ""}`} />
